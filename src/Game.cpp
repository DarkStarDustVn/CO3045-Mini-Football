...
void Game::init(const char *title, int width, int height, bool fullscreen)
{
    ...
    // Load goal sound effect
    goalSound = Mix_LoadWAV("../assets/goal.wav");
    if (goalSound == nullptr) {
        printf("Failed to load goal sound! SDL_mixer Error: %s\n", Mix_GetError());
    }

    // Load and play football crowd music
    music = Mix_LoadMUS("../assets/football_music.wav");
    if (music == nullptr) {
        printf("Failed to load football crowd music! SDL_mixer Error: %s\n", Mix_GetError());
    } else {
...
void Game::update()
{
    ...
    else if (Collision::check(board->getLeftGoalRect(), ball->getRect()))
    {
        if (ball->getVel() != 0)
        {
    ...
    else if (Collision::check(board->getRightGoalRect(), ball->getRect()))
    {
        if (ball->getVel() != 0)
        {
    ...
    else if (Collision::check(board->getTopLeftCornerRect(), ball->getRect())) 
    {
        if (!ball->isColliding) {
            int newDirX = (ballDirX <= 0) ? 1 : ballDirX;
            int newDirY = (ballDirY <= 0) ? 1 : ballDirY;
            ball->setDirs(newDirX, newDirY);
            ball->isColliding = true;
        }
    }
    else if (Collision::check(board->getTopRightCornerRect(), ball->getRect())) 
    {
        if (!ball->isColliding) {
            int newDirX = (ballDirX >= 0) ? -1 : ballDirX;
            int newDirY = (ballDirY <= 0) ? 1 : ballDirY;
    ...
    else if (Collision::check(board->getBottomLeftCornerRect(), ball->getRect())) 
    {
        if (!ball->isColliding) {
            int newDirX = (ballDirX <= 0) ? 1 : ballDirX;
            int newDirY = (ballDirY >= 0) ? -1 : ballDirY;
    ...
    else if (Collision::check(board->getBottomRightCornerRect(), ball->getRect())) 
    {
        if (!ball->isColliding) {
            int newDirX = (ballDirX >= 0) ? -1 : ballDirX;
            int newDirY = (ballDirY >= 0) ? -1 : ballDirY;
